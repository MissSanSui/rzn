...
public class RedisLock implements Lock {
	...
	public void lock() {
		...
		try {
			...
			while ((System.nanoTime() - nowTime) < timeout) {
				...
				if (redisTemplate.getConnectionFactory().getClusterConnection().setNX(key.getBytes(),LOCKED.getBytes())) {
	...
	public boolean tryLock() {
		...
		try {
			...
			// 将锁作为key存储到redis缓存中，存储成功则获得锁
			while ((System.nanoTime() - nowTime) < timeout) {
				if (redisTemplate.getConnectionFactory().getClusterConnection().setNX(key.getBytes(),
						LOCKED.getBytes())) {
	...
	public void unlockExpire() {
		// 5秒释放锁
		redisTemplate.expire(key, 5, TimeUnit.SECONDS);
	}

	public void unlockIm() {
		redisTemplate.delete(key);
	}
